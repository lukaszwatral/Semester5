...
class MLPApproximator(BaseEstimator, RegressorMixin):
    ...
    @staticmethod
    def linear_d(phi_S):
        return np.ones(phi_S.shape, dtype=type(phi_S))
    ...
    def pre_algo_sgd_momentum(self):
        self.v_weights_ = [np.zeros_like(w) if w is not None else None for w in self.weights_]
        self.v_weights0_ = [np.zeros_like(w0) if w0 is not None else None for w0 in self.weights0_]


    def algo_sgd_momentum(self, l):
        self.v_weights_[l] = self.momentum_beta * self.v_weights_[l] + (1-self.momentum_beta) * self.gradients[l]
        self.weights_[l] -= self.learning_rate * self.v_weights_[l]
        self.v_weights0_[l] = self.momentum_beta * self.v_weights0_[l] + (1-self.momentum_beta) * self.gradients0[l]
        self.weights0_[l] -= self.learning_rate * self.v_weights0_[l]

    def pre_algo_rmsprop(self):
        self.sg_weights_ = [np.zeros_like(w) if w is not None else None for w in self.weights_]
        self.sg_weights0_ = [np.zeros_like(w0) if w0 is not None else None for w0 in self.weights0_]

    def algo_rmsprop(self, l):
        self.sg_weights_[l] = self.rmsprop_beta * self.sg_weights_[l] + (1 - self.rmsprop_beta) * self.gradients[l]**2
        self.weights_[l] -= (self.learning_rate/(np.sqrt(self.sg_weights_[l]+self.rmsprop_epsilon))) * self.gradients[l]
        self.sg_weights0_[l] = self.rmsprop_beta * self.sg_weights0_[l] + (1 - self.rmsprop_beta) * self.gradients0[l]**2
        self.weights0_[l] -= (self.learning_rate/(np.sqrt(self.sg_weights0_[l]+self.rmsprop_epsilon))) * self.gradients0[l]

    def pre_algo_adam(self):
        self.m_weights_ = [np.zeros_like(w) if w is not None else None for w in self.weights_]
        self.v_weights_ = [np.zeros_like(w) if w is not None else None for w in self.weights_]
        self.m_weights0_ = [np.zeros_like(w0) if w0 is not None else None for w0 in self.weights0_]
        self.v_weights0_ = [np.zeros_like(w0) if w0 is not None else None for w0 in self.weights0_]

    def algo_adam(self, l):
        self.m_weights_[l] = self.adam_beta1 * self.m_weights_[l] + (1 - self.adam_beta1) * self.gradients[l]
        self.v_weights_[l] = self.adam_beta2 * self.v_weights_[l] + (1 - self.adam_beta2) * self.gradients[l] ** 2
        m__weights_ = self.m_weights_[l] / (1 - self.adam_beta1 ** (self.t + 1))
        v__weights_ = self.v_weights_[l] / (1 - self.adam_beta2 ** (self.t + 1))
        self.weights_[l] -= self.learning_rate * m__weights_ / (np.sqrt(v__weights_) + self.adam_epsilon)
        self.m_weights0_[l] = self.adam_beta1 * self.m_weights0_[l] + (1 - self.adam_beta1) * self.gradients0[l]
        self.v_weights0_[l] = self.adam_beta2 * self.v_weights0_[l] + (1 - self.adam_beta2) * self.gradients0[l] ** 2
        m__weights0_ = self.m_weights0_[l] / (1 - self.adam_beta1 ** (self.t + 1))
        v__weights0_ = self.v_weights0_[l] / (1 - self.adam_beta2 ** (self.t + 1))
        self.weights0_[l] -= self.learning_rate * m__weights0_ / (np.sqrt(v__weights0_) + self.adam_epsilon)
    ...
    def forward(self, X_b):
        ...
        self.signals[0] = X_b
        for l in range(1, len(self.structure) + 2):
            self.signals[l] = self.weights_[l] @ self.signals[l-1].T + self.weights0_[l]
            self.signals[l] = self.signals[l].T
            if l < len(self.structure) + 1:
                self.signals[l] = self.activation_(self.signals[l])
            else:
                self.signals[l] = self.targets_activation_(self.signals[l])
    ...
    def backward(self, y_b):
        ...
        for i in range(len(self.structure) + 1, 0, -1):
            self.gradients[i] = (self.deltas[i].T @ self.signals[i - 1]).astype(np.float32)
            self.gradients0[i] = np.sum(self.deltas[i], axis=0).reshape(-1, 1).astype(np.float32)
            self.deltas[i - 1] = (self.deltas[i] @ self.weights_[i]).astype(np.float32) * self.activation_d_(self.signals[i - 1])
